...
const WorkerNodeConfig: React.FC<WorkerNodeProps> = ({
  isOpen,
  onClose,
  onSubmit,
  clusterName,
  ...
  const getVMName = (): string => {
    ...
    return `${prefix}${clusterName}-worker`;
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success('Worker node provisioned successfully!');
      onSubmit(true);
    } catch (error) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 overflow-y-auto">
      <div className="bg-white rounded-lg shadow-lg w-full max-w-2xl my-8 max-h-[90vh] overflow-y-auto">
        ...
        <div className="sticky top-0 bg-white border-b border-gray-200 p-6">
          <h2 className="text-2xl font-semibold text-gray-900">Configure Worker Node</h2>
          <p className="text-gray-600 mt-1">
            {kubernetesType.charAt(0).toUpperCase() + kubernetesType.slice(1)} Cluster - Worker Node
          </p>
        </div>
          ...
          {loadingNodes ? (
            ...
            <>
              <div className="border border-gray-200 rounded-lg p-6">
                <div className="mb-4">
                  <h3 className="text-lg font-semibold text-gray-900">Worker Node</h3>
                  <p className="text-sm text-gray-600 mt-1">
                    Worker nodes run the containerized workloads in the cluster
                  </p>
                </div>
        ...
        <div className="sticky bottom-0 bg-white border-t border-gray-200 p-6 flex justify-end gap-3">
          ...
          >
            ...
            {provisioning ? 'Provisioning...' : 'Provision Worker'}
          </button>
        </div>
      </div>
    </div>
...
export default WorkerNodeConfig;
